...
PHRASE_THEMES = {"Привет, пока, спасибо", "Давай знакомиться", "Волшебные слова",
                 "Чей? Кому? У кого?",   # падежные формы местоимений — абстракции без картинок
                 "Кто? Что? Какой?",     # вопросительные слова — то же самое
                 "Добрые пожелания"}     # ситуативные формулы: по картинке не различить
...
SORT_EXCLUDED_THEMES = {
    ...
    "Добрые пожелания", "Время и сезоны",
}
...
PACE_BY_THEME = {"Кайда? Где что лежит": 2,
                 # пожелания — формулы из двух слов, воспринимаемые целиком;
                 # по два за урок каждая пара остаётся осмысленной сценкой
                 "Добрые пожелания": 2}
...
async def unit_words(db: AsyncSession, theme_id: int) -> list[Word]:
    # sort_order первичен: у перестроенных тем уроки собраны парами по смыслу,
    # у остальных он нулевой и порядок остаётся прежним, по id
    return (await db.execute(select(Word).where(Word.theme_id == theme_id)
                             .order_by(Word.sort_order, Word.id))).scalars().all()
